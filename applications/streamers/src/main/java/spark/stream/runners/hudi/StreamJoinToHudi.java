...
public class StreamJoinToHudi {
    public static void main(String[] args) throws StreamingQueryException, RestClientException, IOException {
        ...
        Dataset<Row> df1 =  spark
                ...
                .load()
                .select(
                        from_avro(expr("substring(value, 6, length(value)-5)"), valueSchema1Str).alias("value")
                )
                .select("value.*");
        ...
        Dataset<Row> df2 =  spark
                ...
                .load()
                .select(
                        from_avro(expr("substring(value, 6, length(value)-5)"), valueSchema2Str).alias("value")
                )
                .select("value.*");

        Dataset<Row> joinedDf = df1.join(df2, df1.col("user_id").equalTo(df2.col("user_id")), "full_outer")
                .select(
        ...
        Map<String, String> hudiTableOptions = HudiConf.createHudiConf(
                hudiTableName,
                "user_id",
                "event_timestamp1,event_timestamp2",
                ""
        );
