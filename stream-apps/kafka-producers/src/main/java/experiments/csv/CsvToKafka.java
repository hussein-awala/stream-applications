...
public class CsvToKafka {
  public static void main(String[] args) throws IOException, CsvException {
    ...
    InputStream dataInputStream =
        classLoader.getResourceAsStream("data/household_power_consumption.txt");
    CSVParser csvParser = new CSVParserBuilder().withSeparator(';').build();
    try (CSVReader reader =
        new CSVReaderBuilder(new InputStreamReader(dataInputStream))
      ...
      for (String[] row : rows) {
        try {
          ...
          final ProducerRecord<String, GenericRecord> producerRecord =
              new ProducerRecord<>(topicName, null, genericRecord);
