...
public class MessageFakerRunnable implements Runnable {
  ...
  public MessageFakerRunnable(
      String topicName, int waitSeconds, int batchSize, int nbBatchs, String schemaPath) {
  ...
  public MessageFakerRunnable(
      String topicName, int waitSeconds, int batchSize, int nbBatchs, Schema schema) {
    this.topicName = topicName;
    this.waitSeconds = waitSeconds;
    this.batchSize = batchSize;
    this.nbBatchs = nbBatchs;
      ...
      for (int i = 0; i < this.nbBatchs; i++) {
        ...
        for (int j = 0; j < this.batchSize; j++) {
          ...
          final ProducerRecord<String, GenericRecord> record =
              new ProducerRecord<>(
                  this.topicName, String.valueOf(rn.nextInt(1000)), (GenericRecord) message);
