...
public class JoinTable1AndTable2Topics {
    ...
    private static final Logger logger = LogManager.getLogger(JoinTable1AndTable2Topics.class.getName());
    ...
    public Topology buildTopology() throws IOException {
        ...
        final Map<String, String> serdeConfig = Collections.singletonMap("schema.registry.url", JoinTable1AndTable2Topics.schemaRegistryUrl);
        ...
        final KStream<String, GenericRecord> table1Stream = builder.stream(table1Topic, Consumed.with(keyStringSerde, valueGenericAvroSerde));
        final KStream<String, GenericRecord> table2Stream = builder.stream(table2Topic, Consumed.with(keyStringSerde, valueGenericAvroSerde));
        final KTable<Integer, GenericRecord> table1Ktable = table1Stream
                .groupBy((k, v) -> (int) v.get("user_id"), Grouped.with(userIdKeySerde, valueGenericAvroSerde))
                .aggregate(
                        () -> null, (aggKey, newValue, aggValue) -> newValue,
                        Materialized.<Integer, GenericRecord, KeyValueStore<Bytes, byte[]>>as("aggregated-stream-store")
                                .withKeySerde(userIdKeySerde)
                                .withValueSerde(valueGenericAvroSerde)
                );
        KStream<Integer, GenericRecord> joinedStream = table2Stream
