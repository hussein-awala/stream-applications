...
public class Table1TopicTransformer {
  ...
  public Topology buildTopology() {
    ...
    final Map<String, String> serdeConfig =
        Collections.singletonMap("schema.registry.url", Table1TopicTransformer.schemaRegistryUrl);
    final Serde<String> keyStringSerde = Serdes.String();
    final Serde<GenericRecord> valueGenericAvroSerde = new GenericAvroSerde();
    valueGenericAvroSerde.configure(serdeConfig, false);

    StreamsBuilder builder = new StreamsBuilder();
    builder.stream(inputTopic, Consumed.with(keyStringSerde, valueGenericAvroSerde))
